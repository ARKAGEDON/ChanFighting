...
{
	[SerializeField]
	private float walkSpeed = 0.15f;                 // Default walk speed.
	[SerializeField]
	private float runSpeed = 1.0f;                   // Default run speed.
	[SerializeField]
	private float sprintSpeed = 2.0f;                // Default sprint speed.
	[SerializeField]
	private float speedDampTime = 0.1f;              // Default damp time to change the animations based on current speed.
	[SerializeField]
	private string jumpButton = "Jump";              // Default jump button.
	[SerializeField]
	private float jumpHeight = 1.5f;                 // Default jump height.
	[SerializeField]
	private float jumpIntertialForce = 10f;          // Default horizontal inertial force when jumping.
	...
	IEnumerator Dodge()
	{
		...
		{
			isRolling = true;
			behaviourManager.GetAnim.SetBool("Roll", true);
			yield return new WaitForSeconds(1f);
			behaviourManager.GetAnim.SetBool("Roll", false);
			isRolling = false;
		}
	}
}
