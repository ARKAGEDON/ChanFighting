...
{
	public GameObject player;                                           // Player's reference.
	...
	{
		float playerFocusHeight = player.transform.GetComponent<CapsuleCollider> ().height * 0.75f;
	...
	{
		// Cast target.
		Vector3 target = player.transform.position + (Vector3.up * deltaPlayerHeight);
		// If a raycast from the check position to the player hits something...
		if (Physics.SphereCast(checkPos, 0.2f, target - checkPos, out RaycastHit hit, relCameraPosMag))
		{
			// ... if it is not the player...
			if(hit.transform != player.transform && !hit.transform.GetComponent<Collider>().isTrigger)
			{
	...
	{
		// Cast origin.
		Vector3 origin = player.transform.position + (Vector3.up * deltaPlayerHeight);
		if (Physics.SphereCast(origin, 0.2f, checkPos - origin, out RaycastHit hit, maxDistance))
		{
			if(hit.transform != player.transform && hit.transform != transform && !hit.transform.GetComponent<Collider>().isTrigger)
			{
				return false;
			}
		}
